...
url <- paste("http://api.nytimes.com/svc/search/v2/articlesearch.json?q=haaland&api-key=",
             API_KEY, sep ="")
​
...
baseurl <- paste0("http://api.nytimes.com/svc/search/v2/articlesearch.json?q=",term1,"&begin_date=",begin_date,"&end_date=",end_date,"&facet_filter=true&api-key=","NTKBHbsb6XFEkGymGumAiba7n3uBvs8V",
                  sep="")
...
for(i in 0:maxPages){
  ...
  pages[[i+1]] <- nytSearch 
  Sys.sleep(12) #Pauses the request to ask for the desired information to the API
}
...
names(nytDat)
paragraph <- names(nytDat)[6] # The 6th column


tokenized <- nytDat |>
  unnest_tokens(word, paragraph) #the first word of the paragraph 
...
tokenized |> 
  count(clean, sort = T) |> #illegible with all the words displayed
